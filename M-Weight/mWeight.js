...
function calculateMWeight(event) {
  ...
  const sheetSizeL = parseFloat(document.getElementById('sheetSizeL').value);
  const sheetSizeW = parseFloat(document.getElementById('sheetSizeW').value);
  ...
  const result = ((sheetSizeL * sheetSizeW) * basisWeight / (dimensions)) * 2
